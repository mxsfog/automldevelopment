...
with mlflow.start_run(run_name="phase1/step1.4_catboost") as run:
    ...
    try:
        ...
        logger.info("ROI test=%.2f%% val=%.2f%%", roi_test, roi_val)
        ...
        # Сохраняем пайплайн (первая опорная точка)
        import joblib
        ...
        class BestPipeline:
            ...
            def _build_features(self, df: pd.DataFrame) -> pd.DataFrame:
                ...
                feats["ml_winrate_diff"] = df["ML_Winrate_Diff"].fillna(0.0)
                feats["ml_rating_diff"] = df["ML_Rating_Diff"].fillna(0.0)
                feats["hour"] = pd.to_datetime(df["Created_At"], utc=True).dt.hour
                feats["day_of_week"] = pd.to_datetime(df["Created_At"], utc=True).dt.dayofweek
                feats["month"] = pd.to_datetime(df["Created_At"], utc=True).dt.month
                feats["odds_times_stake"] = feats["Odds"] * feats["USD"]
                feats["ml_edge_pos"] = feats["ml_edge"].clip(0)
                feats["ml_ev_pos"] = feats["ml_ev"].clip(0)
                feats["Sport"] = df["Sport"].fillna("unknown")
            ...
            def evaluate(self, df: pd.DataFrame) -> dict:
                ...
                total_payout = selected.loc[won_mask, "Payout_USD"].sum()
                roi = (
                    (total_payout - total_stake) / total_stake * 100 if total_stake > 0 else -100.0
                )
                return {"roi": roi, "n_selected": int(mask.sum()), "threshold": self.threshold}

        pipeline = BestPipeline(
            model=model, feature_names=feats_list, cat_features=cat_features, threshold=bt
        )
        ...
        metadata = {
            ...
            "params": {"depth": 6, "lr": 0.1, "iterations": 500},
            "sport_filter": [],
