"""Step 4.2 — Text features from Outcomes + bet_lead_time + Tournament.

Гипотеза: дополнительные сигналы из текста Outcomes (тип ставки,
bet_lead_time = Start_Time - Created_At, Tournament) улучшат
предсказание относительно базовых ELO+temporal 7.34%.
"""
...
# Паттерны ставок для парсинга текста Outcomes
BET_TYPE_PATTERNS = [
    ("over_under", r"\b[Oo]ver\b|\b[Uu]nder\b"),
    ("handicap", r"\([+-]?\d+\.?\d*\)"),
    ("draw", r"\b[Dd]raw\b|\b[Xx]\b"),
    ("btts", r"\bBTTS\b|\bBoth Teams\b"),
    ("totals", r"\bTotal\b"),
    ("corners", r"\b[Cc]orner"),
    ("cards", r"\b[Cc]ard"),
    ("goals", r"\b[Gg]oal"),
]


def parse_bet_type(outcomes_text: str) -> str:
    """Извлечь тип ставки из текста."""
    if not isinstance(outcomes_text, str):
        return "unknown"
    for bet_type, pattern in BET_TYPE_PATTERNS:
...
def load_data() -> pd.DataFrame:
    """Загрузка с ELO и rich outcomes join."""
...
def build_features(df: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    """ELO + temporal + text-derived фичи."""
    ...
    # Новые фичи: bet_lead_time (как далеко до старта была сделана ставка)
    lead_time = (df["Start_Time"] - df["Created_At"]).dt.total_seconds() / 3600.0
    feats["bet_lead_hours"] = lead_time.clip(-24, 720).fillna(-1)
    feats["is_live_bet"] = (lead_time < 0).astype(int)  # live: ставка после старта
    feats["is_early_bet"] = (lead_time > 24).astype(int)  # ставка более чем за 24ч

    # Тип ставки из текста Outcomes
    feats["bet_type"] = df["Outcomes"].apply(parse_bet_type)

    # Tournament как категориальная фича (top-N по частоте)
    ...
    cat_features = ["Sport", "Market", "Currency", "bet_type", "Tournament"]
...
with mlflow.start_run(run_name="phase4/step4.2_text_features") as run:
    ...
    try:
        ...
        logger.info(
            "Bet types: %s",
            df["Outcomes"].apply(parse_bet_type).value_counts().to_dict(),
        )
        logger.info(
            "Live bets: %.1f%%",
            100 * ((df["Start_Time"] - df["Created_At"]).dt.total_seconds() < 0).mean(),
        )
        ...
        logger.info("ROI val=%.2f%% test=%.2f%%", roi_val, roi_test)
        ...
        mlflow.log_params(
            {
                ...
                "seed": 42,
                "new_features": "bet_lead_hours,is_live_bet,is_early_bet,bet_type,Tournament",
                "depth": 7,
                "half_life": 0.5,
                "threshold": threshold,
        ...
        if roi_test > PREV_BEST_ROI and n_test >= 200:
            ...
            class BestPipeline:
                """Text + ELO + temporal CatBoost pipeline."""
        ...
        print("\n=== Step 4.2 Text Features + ELO + Temporal ===")
        print(f"ROI val={roi_val:.2f}%, test={roi_test:.2f}% ({n_test})")
