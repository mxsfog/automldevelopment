"""Step 3.1 — Optuna TPE hyperparameter optimization.

Гипотеза: Optuna TPE найдёт лучшие гиперпараметры CatBoost
для максимизации ROI на val при ELO+temporal feature set.
"""
...
PREV_BEST_ROI = 7.34  # step 2.1 elo_temporal d7_hl50pct
...
with mlflow.start_run(run_name="phase3/step3.1_optuna") as run:
    ...
    try:
        ...
        logger.info("Data loaded. Starting Optuna with %d trials...", N_TRIALS)

        def objective(trial: optuna.Trial) -> float:
            depth = trial.suggest_int("depth", 4, 9)
            lr = trial.suggest_float("lr", 0.03, 0.3, log=True)
            iterations = trial.suggest_int("iterations", 200, 800)
            half_life = trial.suggest_float("half_life", 0.2, 0.8)
            l2_leaf_reg = trial.suggest_float("l2_leaf_reg", 1.0, 10.0)
            min_data_in_leaf = trial.suggest_int("min_data_in_leaf", 1, 50)

            sw = make_sample_weights(len(train_df), half_life)
            ...
            roi_v, n_v = calc_roi(val_df, pv >= threshold)

            trial.set_user_attr("threshold", threshold)
            trial.set_user_attr("n_val", n_v)
            return roi_v
        ...
        best_roi_val = study.best_value
        best_threshold_val = study.best_trial.user_attrs["threshold"]

        logger.info("Best params: %s", best_params)
        logger.info("Best val ROI: %.2f%% (threshold=%.2f)", best_roi_val, best_threshold_val)
        ...
        pv_final = final_model.predict_proba(X_val)[:, 1]
        pt_final = final_model.predict_proba(X_test)[:, 1]

        auc_v = roc_auc_score(y_val, pv_final)
        auc_t = roc_auc_score(y_test, pt_final)

        threshold = find_best_threshold(val_df, pv_final)
        roi_val, n_val = calc_roi(val_df, pv_final >= threshold)
        roi_test, n_test = calc_roi(test_df, pt_final >= threshold)

        logger.info("Final model: ROI val=%.2f%% test=%.2f%%", roi_val, roi_test)
        ...
        for fold_idx in range(1, 5):
            ...
            sw = make_sample_weights(len(fold_train), best_params["half_life"])
            m_cv = CatBoostClassifier(
                iterations=min(best_params["iterations"], 300),
                learning_rate=best_params["lr"],
                depth=best_params["depth"],
                l2_leaf_reg=best_params["l2_leaf_reg"],
                min_data_in_leaf=best_params["min_data_in_leaf"],
            ...
            m_cv.fit(xft, yft, sample_weight=sw)
        ...
        mlflow.log_metrics(
            {
                "roi_test": roi_test,
                "roi_val": roi_val,
                "roi_val_optuna_best": best_roi_val,
                "roi_mean": roi_mean,
                "roi_std": roi_std,
                "auc_val": auc_v,
                "auc_test": auc_t,
        ...
        if roi_test > PREV_BEST_ROI and n_test >= 200:
            ...
            best_d = best_params["depth"]

            class BestPipeline:
                """Optuna-tuned CatBoost + ELO + temporal."""
        ...
        print(f"\n=== Step 3.1 Optuna HPO ({N_TRIALS} trials) ===")
        print(f"Best Optuna val ROI: {best_roi_val:.2f}% (threshold={best_threshold_val:.2f})")
        print(f"Best params: {best_params}")
        print(f"Final model: ROI val={roi_val:.2f}%, test={roi_test:.2f}% ({n_test})")
