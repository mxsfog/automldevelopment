"""Step 4.8 — chain_8 model + p80 Kelly threshold (higher precision selection).

Гипотеза: Среди HIGH-Kelly бетов (top-20% по Kelly на train) точность
предсказания выше → более чистая выборка → лучший ROI чем p75.

Метод:
- chain_8 model.cbm БЕЗ переобучения (аналогично step 4.6)
- p80 train-Kelly для LOW сегмента = 0.5914 (вычислено в step 4.6)
- Формальный тест: применяем p80 к test ОДИН РАЗ

Дополнительно: анализ подсегментов по odds (1.0-1.3, 1.3-1.5, 1.5-1.8)
чтобы найти более точный сегмент внутри LOW.

Baseline: ROI=28.5833% (n=233)
Step 4.6 best: ROI=30.9146% (n=196, p75 threshold=0.5220)
"""
...
def main() -> None:
    ...
    # chain_8 model.cbm — без переобучения
    ...
    # Kelly на train для вычисления p80 (in-sample, chain_8 model)
    ...
    logger.info(
        "p80 of train LOW Kelly = %.4f (n_train_low=%d)", threshold_p80, len(kelly_train_low)
    )
    ...
    seg_mask_p80 = apply_seg_thresholds(test_df, kelly_test, thresholds_p80)
    roi_p80, n_p80 = calc_roi(test_df, mkt_mask & seg_mask_p80)
    logger.info("p80 threshold=%.4f: ROI=%.4f%% n=%d", threshold_p80, roi_p80, n_p80)

    # Дополнительно: sub-segment анализ внутри LOW (informational)
    sub_bins = [(0, 1.3, "ultra_low"), (1.3, 1.5, "low_mid"), (1.5, 1.8, "low_high")]
    for lo, hi, label in sub_bins:
        sub_mask = (
            (test_df["Odds"].values >= lo)
            & (test_df["Odds"].values < hi)
            & (test_df["Market"].values == "1x2")
            & (kelly_test >= SEGMENT_THRESHOLDS_BASELINE["low"])
        )
        roi_sub, n_sub = calc_roi(test_df, sub_mask)
        logger.info("  sub-seg [%.1f,%.1f) %s: ROI=%.4f%% n=%d", lo, hi, label, roi_sub, n_sub)

    baseline_roi = 28.5833
    delta = roi_p80 - baseline_roi
    delta_vs_p75 = roi_p80 - 30.9146  # delta vs step 4.6

    with mlflow.start_run(run_name="phase4/step_4_8_chain8_p80") as run:
        ...
        mlflow.log_params(
            {
                ...
                "percentile": 80,
                "threshold_p80": round(threshold_p80, 6),
                "threshold_selection": "train_p80_insample",
                "hypothesis": "top20_kelly_higher_precision",
            }
        )
        ...
        try:
            mlflow.log_metrics(
                {
                    ...
                    "n_selected": n_p80,
                    "roi_delta": delta,
                    "roi_delta_vs_p75": delta_vs_p75,
                    "roi_baseline": roi_base,
                }
            )
            ...
            logger.info(
                "RESULT: p80 threshold=%.4f → ROI=%.4f%% n=%d delta=%.4f delta_vs_p75=%.4f",
                threshold_p80,
                roi_p80,
                n_p80,
                delta,
            ...
            print(f"STEP_4_8_ROI={roi_p80:.6f}")
            print(f"STEP_4_8_N={n_p80}")
            print(f"STEP_4_8_DELTA={delta:.4f}")
            print(f"STEP_4_8_DELTA_VS_P75={delta_vs_p75:.4f}")
            print(f"STEP_4_8_THRESHOLD={threshold_p80:.6f}")
            ...
            logger.exception("Ошибка в step 4.8")
            raise
