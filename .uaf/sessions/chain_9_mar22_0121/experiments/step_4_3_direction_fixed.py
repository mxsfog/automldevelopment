"""Step 4.3 — Direction features, полные 500 итераций без early stopping.

Проблема step 4.2/4.2b: early stopping на inflated val → 15 итераций → n=0 бетов.
Решение: обучаем фиксированно 500 итераций (как chain_6 baseline), добавляем
bet_direction (H/D/A/U) как категориальный признак.

Baseline: ROI=28.5833% (n=233), AUC=0.7863
"""
...
def main() -> None:
    ...
    with mlflow.start_run(run_name="phase4/step_4_3_direction") as run:
        ...
        try:
            # Обучаем без early stopping — точно как chain_6 baseline
            ...
            for fname in ["bet_direction", "is_home", "is_draw", "is_away"]:
                logger.info("FI [%s]: %.2f", fname, fi.get(fname, 0))
                mlflow.log_metric(f"fi_{fname}", fi.get(fname, 0))

            # ROI с baseline Kelly thresholds
            ...
            print(f"STEP_4_3_ROI={roi:.6f}")
            print(f"STEP_4_3_N={n_bets}")
            print(f"STEP_4_3_AUC={auc_test:.4f}")
            print(f"STEP_4_3_DELTA={delta:.4f}")
            ...
            logger.exception("Ошибка в step 4.3")
            raise
