"""Step 4.12: Finer odds bins (4-bucket) on 1x2 market + shrunken segment thresholds.

Hypothesis: 1x2 market has 4 natural odds clusters (heavy fav, slight fav, slight underdog,
heavy underdog). Current 3-bin system (low/mid/high at 1.8/3.0) conflates these.
4-bin system [0, 1.6, 2.2, 3.5, inf] may capture distinct calibration patterns.

Bins chosen a-priori (not val-optimized). Shrinkage=0.5 same as best step 4.10.
"""
...
# 4-bin odds breakpoints for 1x2 market (a-priori, not val-optimized)
...
with mlflow.start_run(run_name="phase4/step4.12_1x2_finer_bins") as run:
    try:
        ...
        train_raw = raw_df.iloc[:train_end].copy()
        ...
        logger.info("train=%d, val=%d, test=%d", train_end, len(val_raw), len(test_raw))
        ...
        # Baseline threshold на 1x2 val (a-priori: best single threshold for 1x2)
        t_sweep = np.arange(0.05, 0.95, 0.005)

        baseline_rois = []
        for t in t_sweep:
            mask = val_1x2 & (val_kelly >= t)
            roi, n_sel = calc_roi(val_raw, mask)
            if n_sel >= 5:
                baseline_rois.append((roi, t, n_sel))

        if not baseline_rois:
            raise ValueError("Нет порогов с n>=5 для 1x2 val")

        baseline_t = sorted(baseline_rois, key=lambda x: -x[0])[0][1]
        baseline_val_roi = sorted(baseline_rois, key=lambda x: -x[0])[0][0]
        logger.info(
            "1x2 val baseline threshold=%.3f, val_roi=%.2f%%", baseline_t, baseline_val_roi
        )

        # Поиск raw threshold по каждому бину на 1x2 val
        ...
        for bucket in BIN_LABELS:
            ...
            if bucket_n < 10:
                seg_t_raw[bucket] = baseline_t
                logger.info(
                    "bucket=%s: n=%d < 10, используем baseline_t=%.3f",
                    bucket,
                    bucket_n,
                    baseline_t,
                )
        ...
        # Shrunken thresholds: shrink=0.5 toward 1x2-specific baseline
        seg_t_shrunken = {
            k: baseline_t + SHRINKAGE * (v - baseline_t) for k, v in seg_t_raw.items()
        }
        logger.info("Shrunken thresholds (shrink=%.1f): %s", SHRINKAGE, seg_t_shrunken)
        ...
        logger.info("4-bin shrunken on 1x2: test_roi=%.4f%%, n=%d", roi_4bin, n_4bin)

        # Reference: 3-bin shrunken от step 4.10
        ...
        roi_3bin, n_3bin = calc_roi(test_raw, test_mask_3bin)
        logger.info("3-bin ref on 1x2: test_roi=%.4f%%, n=%d", roi_3bin, n_3bin)

        delta = roi_4bin - roi_3bin
        ...
        for bucket in BIN_LABELS:
            ...
            logger.info("  test bucket=%s: n_sel=%d, roi=%.2f%%", bucket, n_b, roi_b)
        ...
        mlflow.log_metrics(
            {
                "roi_4bin_test": roi_4bin,
                "n_bets_4bin": n_4bin,
                "roi_3bin_ref": roi_3bin,
                "n_bets_3bin": n_3bin,
                "delta_vs_ref": delta,
            }
        )
        ...
        mlflow.set_tag("convergence_signal", "0.88")

        accepted = roi_4bin > roi_3bin
        print(
            f"step4.12 DONE: 4bin_roi={roi_4bin:.4f}%, n={n_4bin}, "
            f"3bin_ref={roi_3bin:.4f}%, delta={delta:+.4f}%, "
