"""Step 3.1 - Optuna hyperparameter optimization.

Hypothesis: Optuna TPE найдёт лучшие гиперпараметры CatBoost
для максимизации ROI на val.
"""
...
def build_features(df: pd.DataFrame, top_200_markets: list[str]) -> tuple[pd.DataFrame, list[str]]:
    """Feature set v3b: market_type + top-200 Market."""
...
with mlflow.start_run(run_name="phase3/step3.1_optuna") as run:
    ...
    try:
        ...
        def objective(trial: optuna.Trial) -> float:
            """Optuna objective: максимизировать ROI на val."""
            params = {
            ...
            threshold = find_best_threshold(val_df, proba_val, min_bets=200)
            mask_val = proba_val >= threshold
            ...
            trial.set_user_attr("n_val", n_val)
            trial.set_user_attr("auc_val", roc_auc_score(y_val, proba_val))
            return roi_val
        ...
        logger.info("Best trial: val ROI=%.2f%%", best_trial.value)
        ...
        logger.info(
            "Final model: AUC val=%.4f test=%.4f, ROI val=%.2f%% (%d), test=%.2f%% (%d)",
            auc_val,
            auc_test,
            roi_val,
            n_val,
        ...
        mlflow.log_metrics(
            {
                ...
                "n_bets_val": n_val,
                "n_bets_test": n_test,
                "optuna_best_val_roi": best_trial.value,
            }
        )
        ...
        print("\n=== Step 3.1 Optuna Results ===")
        print(f"N trials: {N_TRIALS}")
        print(f"Best val ROI (Optuna): {best_trial.value:.2f}%")
