"""Step 4.31 -- Synthesis: combine best findings.

Best findings:
- 4m_p02 (step 4.27): mean=13.82% std=0.90% — most seed-stable
- implied_prior (step 4.30): 66.56% but n=992, 3/6 — ultra-selective
- drop_7 (step 4.23): 14.88% adv_auc=0.721 — reduced drift

Combine: 4-model + implied blend + drop drift features.
Also: grid search over blend alpha and filter thresholds.
"""
...
def main() -> None:
    """Synthesis walk-forward."""
    with mlflow.start_run(run_name="phase4/synthesis") as run:
        try:
            ...
            # Strategies to test
            configs = {
                "3m_baseline": {"n_models": 3, "alpha": 1.0, "ev_th": 0.05, "pstd_th": 0.02},
                "4m_baseline": {"n_models": 4, "alpha": 1.0, "ev_th": 0.05, "pstd_th": 0.02},
                "4m_blend30": {"n_models": 4, "alpha": 0.7, "ev_th": 0.05, "pstd_th": 0.02},
                "4m_blend50": {"n_models": 4, "alpha": 0.5, "ev_th": 0.05, "pstd_th": 0.02},
                "4m_blend30_strict": {
                    "n_models": 4, "alpha": 0.7, "ev_th": 0.05, "pstd_th": 0.015,
                },
                "4m_blend50_strict": {
                    "n_models": 4, "alpha": 0.5, "ev_th": 0.05, "pstd_th": 0.015,
                },
                "3m_blend30": {"n_models": 3, "alpha": 0.7, "ev_th": 0.05, "pstd_th": 0.02},
                "3m_blend50": {"n_models": 3, "alpha": 0.5, "ev_th": 0.05, "pstd_th": 0.02},
            }
            ...
            cumulative: dict[str, dict] = {
                k: {"n": 0, "profit": 0.0} for k in configs
            }
            ...
            for i, test_week in enumerate(retrain_schedule):
                ...
                for c_name, cfg in configs.items():
                    ...
                    preds = (
                        [p_cb, p_lgbm, p_lr] if n_models == 3
                        else [p_cb, p_lgbm, p_lr, p_xgb]
                    )
                    ...
                    if n > 0:
                        ...
                        roi = (payout - n) / n * 100
                        cumulative[c_name]["n"] += n
                        cumulative[c_name]["profit"] += n * roi / 100

                    block_info[f"{c_name}_roi"] = round(roi, 2)
                    block_info[f"{c_name}_n"] = int(n)
            ...
            for c_name in configs:
                total_n = cumulative[c_name]["n"]
                total_profit = cumulative[c_name]["profit"]
                ...
                logger.info(
                    ...
                    c_name, overall_roi, total_n, n_pos, len(all_blocks),
                )
                mlflow.log_metrics(
                    {
                        f"wf_{c_name}_roi": round(overall_roi, 2),
                        f"wf_{c_name}_n": total_n,
                        f"wf_{c_name}_pos": n_pos,
                    }
                )

            res_path = str(SESSION_DIR / "experiments" / "synthesis_results.csv")
