"""Step 4.26 -- XGBoost as 4th model in ensemble.

Current 3-model ensemble: CatBoost + LightGBM + LogisticRegression.
Adding XGBoost may increase diversity and make p_std filter more discriminative.
Test: 3-model vs 4-model, different p_std thresholds.
"""
...
def main() -> None:
    """4-model ensemble walk-forward."""
    with mlflow.start_run(run_name="phase4/xgboost_4model") as run:
        try:
            ...
            # Strategies: ensemble × p_std threshold
            strategies = {
                "3m_p02": {"n_models": 3, "p_std_thresh": 0.02},
                "4m_p02": {"n_models": 4, "p_std_thresh": 0.02},
                "4m_p015": {"n_models": 4, "p_std_thresh": 0.015},
                "4m_p01": {"n_models": 4, "p_std_thresh": 0.01},
                "4m_p025": {"n_models": 4, "p_std_thresh": 0.025},
            }
            ...
            for i, test_week in enumerate(retrain_schedule):
                ...
                for s_name, s_cfg in strategies.items():
                    n_models = s_cfg["n_models"]
                    p_std_thresh = s_cfg["p_std_thresh"]
            ...
            logger.info("\n4-model ensemble results (ev005_agree):")
            ...
            res_path = str(SESSION_DIR / "experiments" / "xgboost_4model_results.csv")
