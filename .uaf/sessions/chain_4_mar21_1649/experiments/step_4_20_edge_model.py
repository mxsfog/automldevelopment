"""Step 4.20 -- Edge-predicting model (residual over implied probability).

Гипотеза: вместо P(win), предсказывать edge = P(win) - P(implied).
Это напрямую таргетирует рыночную неэффективность.

Два подхода:
1. Regression: target = target - 1/odds (continuous edge)
2. Classification: target = 1 where actual win AND odds > 2 (non-trivial wins)
"""
...
def main() -> None:
    """Edge-predicting model walk-forward."""
    with mlflow.start_run(run_name="phase4/edge_model") as run:
        try:
            ...
            mlflow.log_params(
                {
                    ...
                    "seed": 42,
                    "method": "edge_model_residual",
                }
            )

            strategies = [
                "baseline_ev005_agree",
                "reg_edge_pos",
                "reg_edge_pos_capped",
                "clf_implied_edge",
            ]
            ...
            for i, test_week in enumerate(retrain_schedule):
                ...
                # --- Regression: predict PnL per bet ---
                # target: (odds - 1) if won, -1 if lost
                y_train_reg = np.where(
                    train_enc["target"] == 1, train_enc["Odds"].values - 1, -1.0
                )
                ...
                lr_reg.fit(x_train_s, y_train_reg)

                pred_pnl = (
                    cb_reg.predict(x_test) + lgbm_reg.predict(x_test) + lr_reg.predict(x_test_s)
                ) / 3

                # --- Classification on "valuable" bets ---
                # target: 1 if won AND model thinks P > 1/odds (implied edge)
                p_implied_train = 1.0 / train_enc["Odds"].values
                p_train_mean = (
                    cb.predict_proba(x_train)[:, 1]
                    + lgbm.predict_proba(x_train)[:, 1]
                    + lr.predict_proba(x_train_s)[:, 1]
                ) / 3
                edge_target = (
                    (train_enc["target"] == 1) & (p_train_mean > p_implied_train)
                ).astype(int)

                if edge_target.sum() > 50:
                    cb_edge = CatBoostClassifier(
                        iterations=200, learning_rate=0.05, depth=6, random_seed=42, verbose=0
                    )
                    cb_edge.fit(x_train, edge_target)
                    p_edge = cb_edge.predict_proba(x_test)[:, 1]
                else:
                    p_edge = np.zeros(len(x_test))

                block_info = {"block": i, "week": test_week, "n_test": len(test_df)}

                # Evaluate strategies
                strategies_masks = {
                    "baseline_ev005_agree": (ev >= 0.05) & (p_std <= 0.02),
                    "reg_edge_pos": pred_pnl > 0,
                    "reg_edge_pos_capped": (pred_pnl > 0) & (odds <= 10),
                    "clf_implied_edge": p_edge >= 0.5,
                }

                for s_name, mask in strategies_masks.items():
                    n = mask.sum()
                    roi = 0.0
                    if n > 0:
                        sel = test_enc[mask]
            ...
            res_path = str(SESSION_DIR / "experiments" / "edge_model_results.csv")
