"""Step 4.34 -- MLP as 5th model in ensemble.

Current best: 4-model (CB+LGBM+LR+XGB) with blend50. Adding MLP may increase
ensemble diversity further. Test 4m vs 5m, both with blend50.
"""
...
def main() -> None:
    """5-model ensemble walk-forward."""
    with mlflow.start_run(run_name="phase4/mlp_5model") as run:
        try:
            ...
            strategies = {
                "4m_blend50": {"n_models": 4, "alpha": 0.5},
                "5m_blend50": {"n_models": 5, "alpha": 0.5},
                "5m_raw": {"n_models": 5, "alpha": 1.0},
            }
            ...
            for i, test_week in enumerate(retrain_schedule):
                ...
                for s_name, cfg in strategies.items():
                    ...
                    if n_models == 4:
                        ...
                        preds = [p_cb, p_lgbm, p_lr, p_xgb, p_mlp]
            ...
            res_path = str(SESSION_DIR / "experiments" / "mlp_5model_results.csv")
