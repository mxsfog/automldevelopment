"""Step 4.32 -- Robustness of 4m_blend50 (new best strategy).

4m_blend50 = 4-model ensemble, p_final = 0.5*p_model + 0.5*p_implied,
EV >= 0.05, p_std <= 0.02. ROI = 21.80%, 5/6 positive blocks.

Check: seed stability, odds cap sensitivity, and 3m_blend50 comparison.
"""
...
def run_walk_forward(
    ...
    """Single WF pass with blend strategy."""
    random.seed(seed)
    np.random.seed(seed)
...
def main() -> None:
    """Blend robustness check."""
    with mlflow.start_run(run_name="phase4/blend_robustness") as run:
        try:
            ...
            # 1. Seed stability
            logger.info("=== Seed Stability ===")
            for name, n_m, alpha in configs:
                rois = []
                for seed in SEEDS:
                    res = run_walk_forward(
                        df, retrain_schedule, seed, n_m, alpha, None
                    )
                    rois.append(res["roi"])
                    all_results.append({
                ...
                mlflow.log_metrics({
                    ...
                    f"seed_{name}_min": round(min(rois), 2),
                    f"seed_{name}_max": round(max(rois), 2),
                })
            ...
            for name, n_m, alpha in configs:
                for cap in ODDS_CAPS:
                    if cap is None:
                        continue
                    res = run_walk_forward(
                        df, retrain_schedule, 42, n_m, alpha, cap
                    )
                    all_results.append({
                        "config": name, "seed": 42,
                        "odds_cap": str(cap), **res,
                    })
                    logger.info(
                        "  %s cap=%s: roi=%.2f%% n=%d pos=%d/%d",
                        name, cap, res["roi"],
                        res["n"], res["n_pos"], res["n_blocks"],
                    )
                    mlflow.log_metrics({
            ...
            res_path = str(
                SESSION_DIR / "experiments" / "blend_robustness_results.csv"
            )
