"""Step 4.7 — XGBoost: альтернативный GBT с другим bias/calibration.

Гипотеза: XGBoost с tree_method=hist даёт другие вероятности чем CatBoost,
возможно с более высоким threshold → лучший test ROI.
Baseline: ROI=24.91% (CatBoost+Kelly, step 1.4).
"""
...
def build_features(
    ...
    """Feature set с OrdinalEncoder для XGBoost."""
...
def main() -> None:
    """XGBoost baseline + Kelly."""
    with mlflow.start_run(run_name="phase4/step4.7_xgboost") as run:
        ...
        try:
            ...
            # XGBoost аналогичный CatBoost: depth=7, lr=0.1, 500 iter
            model = XGBClassifier(
                max_depth=7,
                learning_rate=0.1,
                n_estimators=500,
            ...
            logger.info(
                "XGBoost: val=%.2f%%, test=%.2f%% (n=%d), t=%.3f, AUC=%.4f, delta=%.2f%%",
                roi_val,
                roi_test,
                n_bets,
                t_best,
            ...
            if roi_test > BASELINE_ROI and n_bets >= 200:
                ...
                model.save_model(str(models_dir / "model.xgb"))
                metadata = {
                    "framework": "xgboost",
