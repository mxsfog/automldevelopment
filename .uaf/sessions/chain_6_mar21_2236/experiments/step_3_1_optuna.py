"""Step 3.1 — Optuna TPE: гиперпараметрный поиск CatBoost.

Гипотеза: Optuna TPE найдёт лучшие гиперпараметры чем дефолтные.
Baseline: ROI=24.91%, depth=7, lr=0.1, iterations=500.
Цель: ROI > 24.91% при n_bets >= 200.
"""
...
class BestPipeline:
    """Пайплайн с Optuna-оптимизированными гиперпараметрами."""
...
def make_objective(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        _t_best, roi_val = find_threshold(val_df, k_v)
        return roi_val
...
def main() -> None:
    """Optuna TPE гиперпараметрный поиск."""
    with mlflow.start_run(run_name="phase3/step3.1_optuna_tpe") as run:
        ...
        try:
            ...
            objective = make_objective(train_df, val_df, x_tr, x_vl, y_tr, y_vl, cat_f, w)
            ...
            # 20 trials Optuna (быстрый поиск)
            study.optimize(objective, n_trials=20, show_progress_bar=False)

            best_params = study.best_params
            best_val_roi = study.best_value
            logger.info("Best Optuna params: %s, val_roi=%.2f%%", best_params, best_val_roi)
            ...
            logger.info(
                "Optuna best: val=%.2f%%, test=%.2f%% (%d bets), t=%.3f, AUC=%.4f, delta=%.2f%%",
                roi_val,
                roi_test,
                n_bets,
                t_best,
            ...
            mlflow.log_metrics(
                {
                    ...
                    "optuna_best_val_roi": float(best_val_roi),
                }
            )

            # Логируем все триалы
            for i, trial in enumerate(study.trials):
                mlflow.log_metric("optuna_trial_roi", trial.value or -999, step=i)
            ...
            print(
                ...
                f"best_params={best_params} run_id={run.info.run_id}"
            )
