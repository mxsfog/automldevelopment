"""Step 3.2 — Optuna TPE с AUC-метрикой (без двойного переобучения).

Проблема step 3.1: Optuna максимизировал val ROI (включая threshold search) → overfit.
Решение: Optuna максимизирует val AUC. Threshold выбирается независимо после Optuna.
"""
...
class BestPipeline:
    """Пайплайн с Optuna-AUC-оптимизированными гиперпараметрами."""
...
def main() -> None:
    """Optuna TPE с AUC: без double-dipping."""
    with mlflow.start_run(run_name="phase3/step3.2_optuna_auc") as run:
        ...
        try:
            ...
            def objective(trial: optuna.Trial) -> float:
                ...
                # Оптимизируем AUC — не ROI (нет threshold search внутри Optuna)
            ...
            logger.info(
                "Optuna AUC: val=%.2f%%, test=%.2f%% (%d bets), t=%.3f, AUC=%.4f, delta=%.2f%%",
                roi_val,
                roi_test,
                n_bets,
                t_best,
            ...
            for i, trial in enumerate(study.trials):
                mlflow.log_metric("optuna_trial_auc", trial.value or 0.0, step=i)
