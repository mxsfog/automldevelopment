"""Step 1.4 — CatBoost default + Kelly: strong non-linear baseline.

Воспроизводит chain_1/chain_3 конфигурацию. Ожидаемый ROI ~24.91%.
"""
...
def main() -> None:
    """CatBoost default + Kelly threshold."""
    with mlflow.start_run(run_name="phase1/step1.4_catboost_default") as run:
        ...
        try:
            ...
            logger.info(
                "CatBoost Kelly: val=%.2f%%, test=%.2f%% (%d bets), t=%.3f, AUC=%.4f",
                roi_val,
                roi_test,
                n_bets,
                t_best,
