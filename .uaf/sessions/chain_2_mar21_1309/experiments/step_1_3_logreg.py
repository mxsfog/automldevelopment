...
Базовые числовые фичи, порог по val.
"""
...
def load_and_split() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for col in ["USD", "Payout_USD", *FEATURES]:
...
def main():
    ...
    with mlflow.start_run(run_name="phase1/step_1_3_logreg") as run:
        try:
            ...
            y_train = (train["Status"] == "won").astype(int).values
            y_test = (test["Status"] == "won").astype(int).values
            ...
            thr = find_best_threshold(proba_val, val)
            test_result = calc_roi(test, proba_test >= thr)
            logger.info(
