"""Step 2.2 — New features beyond chain_1.

Shadow feature trick: baseline (chain_1 features) vs candidate (+new features).
New features:
- edge_abs: abs(ML_Edge), strength of model signal
- odds_sq: Odds^2, non-linear odds interaction
- elo_winrate_product: team_elo * team_winrate, combined strength
- streak_x_elo: recent_win_streak * team_elo, momentum x strength
- value_edge: value_ratio * ML_Edge, combined value signal
- games_log: log(team_games+1), reliability of stats
- is_esports: binary for esports vs traditional sports
- odds_edge_ratio: ML_Edge / Odds, normalized edge
"""
...
BASELINE_FEATURES = BASELINE_NUM + CAT_FEATURES
CANDIDATE_FEATURES = BASELINE_NUM + NEW_NUM + CAT_FEATURES
...
def train_and_eval(
    ...
    """Train CatBoost, return (roi, auc, n_bets, threshold)."""
    ...
    logger.info("[%s] AUC: %.4f, iters: %d", label, auc, model.tree_count_)
    ...
    logger.info("[%s] ROI=%.2f%% (%d bets, thr=%.2f)", label, best_roi, best_n, best_thr)
    ...
    return best_roi, auc, best_n, best_thr, model
...
def main():
    ...
    with mlflow.start_run(run_name="phase2/step_2_2_new_features") as run:
        try:
            ...
            bl_roi, bl_auc, bl_n, bl_thr, _ = train_and_eval(
                train, val, test, BASELINE_FEATURES, CAT_FEATURES, "baseline"
            )
            cd_roi, cd_auc, cd_n, cd_thr, cd_model = train_and_eval(
                train, val, test, CANDIDATE_FEATURES, CAT_FEATURES, "candidate"
            )

            delta = cd_roi - bl_roi
            decision = "accept" if delta > 0.2 else ("marginal" if delta > 0 else "reject")
            logger.info(
                "BASELINE: ROI=%.2f%%, CANDIDATE: ROI=%.2f%%, delta=%.2f%%, decision=%s",
                bl_roi,
                cd_roi,
                delta,
                decision,
            ...
            mlflow.log_params(
                {
                    ...
                    "new_features": ",".join(NEW_NUM),
                    "decision": decision,
                    "threshold": cd_thr,
                }
            )

            best_roi = max(bl_roi, cd_roi)
            mlflow.log_metrics(
                {
                    "roi": best_roi,
                    "roi_baseline": bl_roi,
                    "roi_candidate": cd_roi,
                    "delta_roi": delta,
                    "auc_baseline": bl_auc,
                    "auc_candidate": cd_auc,
                    "n_bets": cd_n if cd_roi >= bl_roi else bl_n,
                    "threshold": cd_thr if cd_roi >= bl_roi else bl_thr,
                }
            )
            ...
            if cd_roi > 5.60:
                ...
                cd_model.save_model(str(models_dir / "model.cbm"))
                metadata = {
                    "framework": "catboost",
