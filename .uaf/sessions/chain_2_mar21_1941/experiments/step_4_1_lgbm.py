"""Step 4.1 - LightGBM baseline.

Гипотеза: LightGBM с теми же фичами что CatBoost (step 1.4) покажет сопоставимый
или лучший ROI за счёт другой регуляризации и скорости обучения.
Дополнительно: тестируем более тонкий threshold search (шаг 0.01 вместо 0.02).
"""
...
def build_features(df: pd.DataFrame, encoders: dict | None = None) -> tuple[pd.DataFrame, dict]:
    """Feature engineering для LightGBM с label-encoded категориями."""
    ...
    # LightGBM: нативные категории через category dtype
    fit_mode = encoders is None
    if fit_mode:
        encoders = {}

    for col in ["Sport", "Market", "Currency"]:
        ...
        if fit_mode:
            ...
            enc = encoders[col]
            # unseen categories → -1
            raw_mapped = raw.map(lambda x, c=enc: c.transform([x])[0] if x in c.classes_ else -1)
            feats[col] = raw_mapped.astype(int)
            continue
        feats[col] = enc.transform(raw).astype(int)
...
def find_best_threshold(val_df: pd.DataFrame, proba: np.ndarray, min_bets: int = 200) -> float:
    """Найти лучший порог на val-сете (шаг 0.01)."""
...
with mlflow.start_run(run_name="phase4/step4.1_lgbm") as run:
    ...
    try:
        ...
        X_train, encoders = build_features(train_df)
        X_val, _ = build_features(val_df, encoders)
        X_test, _ = build_features(test_df, encoders)
        ...
        logger.info("Фичи: %d, обучаем LightGBM...", X_train.shape[1])
        ...
        roi_val, n_val = calc_roi(val_df, proba_val >= best_threshold)
        roi_test, n_test = calc_roi(test_df, proba_test >= best_threshold)
        logger.info("ROI val=%.2f%% (%d), test=%.2f%% (%d)", roi_val, n_val, roi_test, n_test)
        ...
        for fold_idx in range(1, 5):
            ...
            x_ft, enc_fold = build_features(fold_train)
            x_fv, _ = build_features(fold_val_cv, enc_fold)
            y_ft = (fold_train["Status"] == "won").astype(int)

            for col in cat_cols:
                x_ft[col] = x_ft[col].astype("category")
                x_fv[col] = x_fv[col].astype("category")
            ...
            m_fold.fit(x_ft, y_ft)

            pf = m_fold.predict_proba(x_fv)[:, 1]
            ...
            logger.info("CV Fold %d: ROI=%.2f%% (%d ставок)", fold_idx, roi_f, n_f)
        ...
        # Сохраняем если лучший результат
        if roi_test > PREV_BEST_ROI and n_test >= 200:
            models_dir = Path(SESSION_DIR) / "models" / "best"
            models_dir.mkdir(parents=True, exist_ok=True)

            enc_ref = encoders  # замыкание для pipeline

            class BestPipeline:
                """Полный пайплайн LightGBM."""

                def __init__(self, model, feature_names, encoders, threshold, framework="lgbm"):
                    self.model = model
                ...
                def _build_features(self, df: pd.DataFrame) -> pd.DataFrame:
                    feats, _ = build_features(df, self.encoders)
                    cat_cols = ["Sport", "Market", "Currency"]
                    for col in cat_cols:
                ...
                def predict_proba(self, df: pd.DataFrame) -> np.ndarray:
                    return self.model.predict_proba(self._build_features(df))[:, 1]
            ...
            model.booster_.save_model(str(models_dir / "model.lgb"))
