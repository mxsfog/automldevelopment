...
Гипотеза: CatBoost с дефолтными параметрами + Kelly threshold + pre-match filter
должен превысить LogReg baseline и приблизиться к исторически лучшему 24.91%.

Воспроизведение pipeline из chain_1 для подтверждения baseline.
"""
...
def load_data() -> pd.DataFrame:
    """Загрузка и объединение всех файлов данных."""
...
def build_features(df: pd.DataFrame) -> tuple[pd.DataFrame, list[str]]:
    """Фичи идентичные chain_1 step 4.5/4.10."""
...
def main() -> None:
    ...
    with mlflow.start_run(run_name="phase1/step1.4_catboost_default") as run:
        ...
        try:
            ...
            logger.info("AUC val/test: %.4f / %.4f", auc_val, auc_test)
            ...
            logger.info(
                "CatBoost Kelly: val=%.2f%%, test=%.2f%% (%d bets), t=%.3f",
                roi_val,
                roi_test,
                n_bets,
                t_best,
            ...
            if roi_test > LEAKAGE_THRESHOLD:
                logger.error(
                    "LEAKAGE SUSPECT: roi_test=%.2f > %.1f%%", roi_test, LEAKAGE_THRESHOLD
                )
            ...
            # Сохраняем как best pipeline если новый рекорд
            ...
            (models_dir / "metadata.json").write_text(_json.dumps(metadata, indent=2))
            ...
            print(
                ...
                f"n_bets={n_bets} auc_test={auc_test:.4f} run_id={run.info.run_id}"
            )
